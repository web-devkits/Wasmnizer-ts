        ...
        switch (node.kind) {
            ...
            case ts.SyntaxKind.CallExpression: {
                ...
                if (isTypeGeneric(origType) && originalFunctionScope) {
                    ...
                    // there is a specialization parameter list
                    if (typeArguments.length > 0) {
                        ...
                        if (
                            ...
                            // create a new specialized method type
                            const specializedType =
                                TypeResolver.createSpecializedType(
                                    origType,
                                    typeArguments,
                            ...
                            const optional =
                                classType.getMethod(origScopeName).method!
                                    .optional;
                            classType.addMethod({
                                name: newScopeName,
                            ...
                            classType.overrideOrOwnMethods.add(newScopeName);

                            const drivedClasses = classType.getDrivedClasses();
                            if (drivedClasses) {
                                drivedClasses.forEach((c) => {
                                    if (
                                        c.overrideOrOwnMethods.has(
                                            origScopeName,
                                        )
                                    ) {
                                        const originalMethod =
                                            c.getMethod(origScopeName).method!;
                                        const originalFuncType =
                                            originalMethod.type;
                                        const specializedFuncType =
                                            TypeResolver.createSpecializedType(
                                                originalFuncType,
                                                typeArguments!,
                                                originalFuncType,
                                            ) as TSFunction;
                                        specializedFuncType.setSpecializedArguments(
                                            typeArguments,
                                        );
                                        createFunctionScopeByFunctionType(
                                            originalFuncType.belongedScope as FunctionScope,
                                        ...
                                        c.addMethod({
                                            name: newScopeName,
                                            type: specializedFuncType,
                                            optional: optional,
                                        });
                                        c.overrideOrOwnMethods.add(
                                            newScopeName,
                                        );
                                    }
                                });
